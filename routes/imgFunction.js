...
const supabase = createClient(process.env.SUPABASE_WEBSITE,process.env.SUPABASE_KEY)
...
const uploadImg = async (file,username,id,Img,foldername="post") => {
  ...
  const filename = `${username}/${foldername}/${fileDetails.name}${username}${id}${Math.floor(Math.random()*1000)}${fileDetails.ext}`
